...
var auth="1w4N8xEi9IMdvNBxLeu9641HmIq";
...
async function insertGameByUrlv2(url,userId){
    ...
    var checkGame=(await new db().selectQuery({  i : i },"Games") )
    if ( checkGame && checkGame.length>0  ){
        return false;
    }
    ...
    try {
        ...
        try {
            await new db().insert({gameId:gameId,userId:userId},"Game_Manual")
        } catch (error) {
            console.log(error)
        ...
        for (ver of obj.versionHistory)  {
            var checkVersino=(await new db().selectQuery({  version_name : ver.versionDisplay , gameId : gameId },"Versions") )
            if ( checkVersino && checkVersino.length>0  ){
                 continue;
            }
            await new db().insert({ gameId:gameId,version_text:ver.releaseNotes,version_name:ver.versionDisplay, version_date:ver.releaseDate},"Versions")
            console.log("version added version:"+ver.versionDisplay +" game id:"+gameId )
            if(gameId){
                var re=await new db().selectQuery({targetId:gameId ,typeId:2 },"Notification_Request")
                if ( re && re.length>0  ){
                    for (ri of re) {
                        await new db().insert({date:new Date().toISOString(),userId:ri.userId,text:`<a href="/game/${re[0].targetId }">${insertObj.n}</a> yeni versiyonu çıktı (${ver.versionDisplay}) <a class="btn" href="/game/${re[0].targetId}"><i class="fas fa-link"></i></a>`},"Notifications");
                    }
                }
            }
            if(insertObj.companyId){
                var re=await new db().selectQuery({targetId:insertObj.companyId ,typeId:3 },"Notification_Request")
                if ( re && re.length>0  ){
                    for (ri of re) {
                        await new db().insert({date:new Date().toISOString(),userId:ri.userId,text:`Yayıncı <a href="/company/${re[0].targetId }">${insertObj.dn}</a>, <a href="/game/${gameId}">${insertObj.n}</a> oynunun yeni versiyonunu çıkarttı (${ver.versionDisplay}) <a class="btn" href="/game/${gameId}"><i class="fas fa-link"></i></a>`},"Notifications");
                    }
                }
            }
        }
...
async function insertCompanyById(di,text){
    var checkCompany=(await new db().selectQuery({  di : di },"Companys") )
    if ( checkCompany && checkCompany.length>0  ){
        return
    }else{
        await new db().insert({ text:text,di: di },"Companys");
...
async function paramC(){
    var ccc={
        "1O37PodgK24biET9lBioEBugMk3": "Amanotes Pte. Ltd.",
      }
    for ( key in ccc) {
        try {
            await insertCompanyById(key,ccc[key])
        } catch (error) {
